...
- Imports lite6_isaac.urdf
- Finds articulation root + link_eef prim
- Runs random joint-delta actions for N steps
- Prints distance-to-target stats

This validates the sim control loop before DreamerV3 training.
"""
...
def main():
    ...
    if not art.handles_initialized:
        # try to locate a real articulation root in stage
        alt = find_prim_path_by_suffix(stage, "UF_ROBOT")
        if alt:
            art = Articulation(prim_path=alt)
            art.initialize()

    if not art.handles_initialized:
        print(f"ERROR: {stage_path} is not an articulation (handles not initialized)")
        app.close()
        return 2
    ...
    if dof < 6:
        print("ERROR: expected at least 6 DOF")
        app.close()
        return 2
    ...
    for t in range(N):
        q = np.array(art.get_joint_positions(), dtype=np.float32)[idx]
        a = rng.uniform(-delta_lim, delta_lim, size=(6,)).astype(np.float32)
        q2 = q + a
        # crude clip
        q2 = np.clip(q2, -3.14, 3.14)

        full = np.array(art.get_joint_positions(), dtype=np.float32)
        full[idx] = q2
        ...
        if (t + 1) % 50 == 0:
            print(f"step={t+1} dist={dist:.4f} ee={ee.tolist()} target={target.tolist()}")
